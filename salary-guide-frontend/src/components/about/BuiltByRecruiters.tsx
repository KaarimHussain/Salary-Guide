...
const BuiltByRecruiters = () => (
    <section className="py-20 px-6 bg-background">
        <div className="max-w-3xl mx-auto text-center">
            <p className="text-xs font-semibold tracking-widest text-muted-foreground uppercase mb-4">Our Story</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-4">Built by recruiters who understand what marketers need</h2>
            <p className="text-muted-foreground text-sm mb-10">After years of placing marketing talent, our team kept hearing the same questions:</p>
            <div className="flex flex-col gap-4 mb-10">
                {questions.map((q, i) => (
                    <div key={i} className="flex items-center justify-center gap-3 bg-muted/50 rounded-2xl px-6 py-4 border border-border">
                        <span className="text-2xl">{["💰", "🏢", "✨"][i]}</span>
                        <p className="text-foreground font-medium text-sm italic">"{q}"</p>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {features.map(({ icon: Icon, label, sub, color }) => (
                    <div key={label} className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-border bg-card">
