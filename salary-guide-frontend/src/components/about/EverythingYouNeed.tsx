...
const EverythingYouNeed = () => (
    <section className="py-20 px-6 bg-muted/30">
        <div className="max-w-4xl mx-auto text-center">
            <p className="text-xs font-semibold tracking-widest text-muted-foreground uppercase mb-3">For Marketers</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3">Everything you need to advance your marketing career</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {needs.map(({ tag, title, desc, icon: Icon, color }) => (
                    <div key={tag} className="bg-card rounded-2xl border border-border p-6 text-left flex flex-col gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${color}`}>
                            <Icon className="w-5 h-5" />
                        </div>
                        <div>
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">{tag}</p>
